...
def fetch_git_repo(git_url: str, commit: str) -> str:
    ...
    # Disable git-lfs filters if git-lfs binary is not available
    no_lfs = ["-c", "filter.lfs.process=", "-c", "filter.lfs.smudge=cat", "-c", "filter.lfs.required=false"]
    if not os.path.isdir(os.path.join(clone_dir, ".git")):
        subprocess.run(["git"] + no_lfs + ["clone", "--depth=1", git_url, clone_dir], check=True)
    ...
    if head[:COMMIT_LEN] != commit[:COMMIT_LEN]:
        subprocess.run(["git"] + no_lfs + ["fetch", "origin", commit], cwd=clone_dir, check=True)
        try:
            subprocess.run(["git"] + no_lfs + ["checkout", commit], cwd=clone_dir, check=True)
